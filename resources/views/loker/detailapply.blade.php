...
@section('content')
	...
	<form action="/loker/listapply/{{$getid->id}}" method="GET" class="search">
        ...
        <input type="text" class="form-control col-form-input" name="search" value="{{request('search')}}">
    ...
    <table class="table table-responsive  data-table">
	    <thead>
            <tr class="judul">
                ...
                <th scope="col" width="1%">Education</th>
                <th scope="col">Position Applied</th>
                <th scope="col">Placement</th>
                <th scope="col">Progres</th>
                <th scope="col" width="10%" colspan="4">Actions</th>
	        </tr>
	   </thead>
	   <tbody >
	   @forelse ($list as $det)
           ...
           @include('loker.modalapprove')
            <tr class="isi">
	            ...
	            <td class="name">{{ Str::limit($det->pelamar->nama_lengkap, 20) }}</td>
...
-->
		...
		<a href="{{ route('verif.createemploye', $det->id) }}" class="btn btn-sm btn-primary">Contract</a>
               ...
               @else   
	     <button class="btn btn-sm btn-success btn-approve" data-toggle="modal" data-target="#exampleModal{{$det->id}}" data-id ="{{$det->id}}" data-whatever="@mdo">
